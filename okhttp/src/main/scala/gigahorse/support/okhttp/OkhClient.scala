...
class OkhClient(config: Config) extends HttpClient {
  ...
  def buildRequest(request: Request): (XRequest, XOkHttpClient) = {
    ...
    val u0 = Option(HttpUrl.parse(httpUrl)).getOrElse {
      sys.error(s"'$url' is not a well-formed URL")
    }
    ...
    def bdy: RequestBody = body match {
      ...
      case b: InMemoryBody =>
        RequestBody.create(
          MediaType.parse(contentType.getOrElse("text/plain; charset=utf-8")),
          b.bytes
        )
      case b: FileBody =>
        RequestBody.create(
  ...
  def download(request: Request, file: File): Future[File] =
    processStream(
      request,
      OkHandler.stream { (res: FullResponse) =>
        Future.successful {
  ...
  def configureSsl(
      ...
      b1: XOkHttpClient.Builder
  ): XOkHttpClient.Builder = {
    // context!
    if (sslConfig.default) b1
      ...
      val b3 =
        if (sslConfig.loose.disableHostnameVerification)
          b2.hostnameVerifier(SSL.insecureHostnameVerifier)
        else b2
      b3
    }
  }

  def configureSignatureCalculator(
    ...
    b1.addNetworkInterceptor(new Interceptor {
      override def intercept(chain: Interceptor.Chain): XResponse = {
        ...
        val signedRequest =
          request.newBuilder.header(name, value).method(request.method, request.body).build
